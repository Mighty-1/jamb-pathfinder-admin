...
export const getInstituteTypes = () =>
  api.get("/institute-types/get-institute-types"); // or /institute-types
export const getStates = () => api.get("/states/get-states"); // or /states
export const getInstitutesByState = (stateId: string, typeId: string) =>
  api.get(`/institutes/get-institutes?stateId=${encodeURIComponent(stateId)}&typeId=${encodeURIComponent(typeId)}`); // or /institutes?stateId=...
export const getCoursesByInstitute = (instituteId: string) =>
  api.get(`/courses/get-courses-by-institute?instituteId=${encodeURIComponent(instituteId)}`); // or /courses?instituteId=...
export const getCourseSubjects = (courseId: string) =>
  api.get(`/courses/subjects/${encodeURIComponent(courseId)}`); // based on earlier scaffolding
...
export const updateCourseSubjects = (id: string, payload: any) =>
  api.patch(`/courses/${encodeURIComponent(id)}/subjects`, payload); // Not yet Available
